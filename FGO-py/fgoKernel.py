            ...
            while True:
                if Detect(.3,.3).isMainInterface():
                    while not Detect(.3,.3).isChooseFriend():
                        fgoDevice.device.touch((p[0],p[1]+75))if(p:=Detect.cache.getNextLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getCloseLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getCrossLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getStartQuestLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getStartLoc())else fgoDevice.device.perform('\x08K',(1000,300))if Detect.cache.isStorySkip()else fgoDevice.device.touch(p)if(p:=Detect.cache.getDialogLoc())else(p:="BattleBegin")if Detect.cache.isBattleBegin()else fgoDevice.device.perform('\xBB\x08',(100,100))if Detect.cache.isSpecialDropRainbowBox()else fgoDevice.device.perform('\x1B',(300,))if Detect.cache.isSpecialDropSuspended()else(p:="TurnBegin")if Detect.cache.isTurnBegin()else None
                        if Detect(.7,.3).isApEmpty()and not self.eatApple():return
