    ...
    def __call__(self):
        ...
        while True:
            ...
            while True:
                if Detect(.3,.3).isMainInterface():
                    if Detect(.7,.3).isApEmpty()and not self.eatApple():return
                    while not Detect(.3,.3).isChooseFriend():fgoDevice.device.touch((p[0],p[1]+75))if(p:=Detect.cache.getNextLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getCloseLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getCrossLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getStartQuestLoc())else fgoDevice.device.touch(p)if(p:=Detect.cache.getStartLoc())else fgoDevice.device.perform('\x08K',(300,300))if Detect.cache.isStorySkip()else fgoDevice.device.touch(p)if(p:=Detect.cache.getDialogLoc())else fgoDevice.device.perform(' M ',(2000,2000,10000))if Detect.cache.isBattleBegin()else fgoDevice.device.perform("X",(300,))if Detect.cache.isSpecialDropRainbowBox()else None
                    self.chooseFriend()
