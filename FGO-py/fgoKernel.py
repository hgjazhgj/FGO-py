...
class Turn:
    ...
    @logit(logger,logging.INFO)
    def selectCard(self):
        color,sealed,hougu,resist,critical,group=[[1,.8,1.1][i]for i in Detect().getCardColor()+[i[5][1]for i in self.servant]],Detect.cache.isCardSealed(),Detect.cache.isHouguReady(),[[1,1.7,.6][i]for i in Detect.cache.getCardResist()],[i/10 for i in Detect.cache.getCardCriticalRate()],(lambda x:Detect.cache.getCardServant(x)+x)([i[0]for i in self.servant])
        ...
        hougu=[i+4 for i in houguSupport+houguArea+houguTargeted]
        ...
        # if sum(self.enemy)>0 and self.enemy[self.target]==0:self.target=next(i for i in range(5,-1,-1)if self.enemy[i])
        # for i in houguTargeted:
            

        def evaluate(card):
            target=self.target
            enemy=self.enemy
            braveChain=len(set(group[i]for i in card))==1
            colorChain={(0,):0,(1):1,(2,):2,(0,1,2):3}.get(tuple(set(color[i]for i in card)),-1)
            firstBonus=7 if colorChain==3 else 1<<color[0]
            result=0
            

        # card=max(permutations(range(5),3-len(hougu)),key=lambda x:evaluate(hougu+x))
