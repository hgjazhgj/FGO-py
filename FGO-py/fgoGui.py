...
class MainWindow(QMainWindow,Ui_fgoMainWindow):
    ...
    def funcEnd(self,msg):
        ...
        def materialText():return "<br/>".join(f"""<img src="fgoImage/material/{i}.png" height="18" width="18">{QApplication.translate("material",i)}{self.color(0x7030A0)}x{j}</font>"""for i,j in self.result["material"].items())if self.result["material"]else self.tr("无")
        match self.result:
