...
from textual.widgets import Button as TextualButton,Checkbox,Input,Select,OptionList,RichLog,Static,TextArea,Footer
...
class Translator:
    ...
    def __call__(self,context,value):
        value='-'.join(map(str,value))if isinstance(value,tuple)else str(value)
...
def activate(widget):
    ...
    widget.focus()
    if isinstance(widget,Button):widget.press()
    elif isinstance(widget,Checkbox):widget.toggle()

class Button(TextualButton):
    BINDINGS=[Binding('space','press',show=False)]
    def __init__(self,label,key,callback,**kwargs):
        super().__init__(**kwargs)
        self.label=f'{self.app.tr("fgoMainWindow",label)} ({key.upper()})'
        self.hotkey,self.callback=key,callback
...
class Popup(ModalScreen):
    ...
    def on_key(self,event):
        if isinstance(self.focused,(Input,TextArea))or not event.character:return
        for widget in self.query('*'):
            if getattr(widget,'hotkey',None)==event.character.lower():
                event.stop()
                event.prevent_default()
                activate(widget)
                return
...
class ChoiceDialog(Popup):
    def __init__(self,title,items,index=0):
        ...
        self.number=Number('#','n',(self.list.highlighted or 0)+1,1,len(items),select_on_focus=False)
    ...
    def on_key(self,event):
        if self.focused is self.list and event.key=='space':
            ...
            return
        if self.focused is self.list and event.character and event.character in'0123456789':
            event.stop()
            event.prevent_default()
            self.number.focus()
            self.number.value=event.character
...
class DeviceDialog(Popup):
    def __init__(self,devices,value):
        ...
        self.list=hotkey(OptionList(*map(Text,self.devices),classes='choices',disabled=not self.devices),'d')
...
class MainWindow(App):
    ...
    BINDINGS=[Binding(f'alt+{i}',f'hotkey("{i}")',show=False,priority=True)for i in'abcdefghijklmnopqrstuvwxyz0123456789']+[
        Binding('f1','help','HELP',priority=True),Binding('f10','menu','MENU',priority=True),
...
#LBL_DEVICE { min-height: 3; content-align: center middle; }
...
.viewer { height: 16; min-height: 3; }
.choices { height: 12; border: round $primary; }
DeviceDialog .choices { height: auto; min-height: 3; max-height: 12; }
ExecuteDialog > .dialog { width: 95%; max-width: 95%; height: 95%; max-height: 95%; }
ExecuteDialog > .viewer { height: 1fr; }
ScreenshotDialog > .dialog { width: 95%; max-width: 95%; height: 95%; max-height: 95%; }
...
'''
    ...
    def build(self):
        ...
        self.BTN_QUESTADD=Button('+','i',self.questAdd)
        self.BTN_QUESTREMOVE=Button('-','d',self.questRemove)
        self.BTN_QUESTUP=Button('↑','u',lambda:self.questMove(-1))
        self.BTN_QUESTDOWN=Button('↓','j',lambda:self.questMove(1))
        self.BTN_QUESTCLEAR=Button('×','e',self.questClear)
        ...
        self.BTN_STOP=Button('终止战斗','9',self.stop,disabled=True)
        self.BTN_STOPLATER=Button('预约终止','l',self.stopLater,disabled=True)
        self.BTN_SCREENSHOT=Button('检查截图','0',self.screenshot)
        self.LST_QUEST=hotkey(OptionList(id='LST_QUEST'),'1')
        self.LST_QUEST.border_title=f'{self.tr("fgoMainWindow","关卡队列")} (1)'
        self.LST_QUEST.tooltip='Enter/Space: # → Enter'
        self.LOG=hotkey(RichLog(id='LOG',min_width=1,wrap=True,max_lines=500),'2')
        self.LOG.border_title='Log (2)'
        for widget,text in(
    ...
    def compose(self):
        ...
        yield Footer()
    def on_mount(self):
        for key,action,text in(('f1','help','HELP'),('f10','menu','MENU'),('ctrl+c','stop','终止战斗'),('ctrl+q','quit','退出')):
    ...
    def action_hotkey(self,key):
        for widget in self.screen.query('*'):
            if getattr(widget,'hotkey',None)==key and not widget.disabled:
                return self.invoke(lambda:activate(widget))
        if key=='x'and len(self.screen_stack)==1:self.askQuit()
    def action_menu(self):
        if isinstance(self.screen,Popup):self.screen.dismiss(None)
        else:self.menu('file')
    ...
    def resultText(self,result):
        ...
        sentence=lambda *parts:(' 'if self.tr.language=='en'else'').join(map(tr,parts))
        lines=[]
        match result:
...
https://paypal.me/hgjazhgjpp''')
    ...
    def help(self):
        self.message('HELP','\n'.join(self.tr('MainWindow',text)for text in(
            'Tab / Shift+Tab：切换焦点；Alt+括号内字符：操作按钮、复选框或聚焦控件',
            'Enter / Space：操作按钮；Space：切换复选框、展开下拉框',
            '下拉框：方向键切换；展开后输入编号，Enter 确认',
            '数值框：直接输入；左右移动光标，上下加减；Space 弹窗编辑，Esc 恢复原值',
            '弹窗：Esc / Alt+C 取消，Alt+O 确认',
            'F10：菜单/关闭弹窗；Alt+X / Ctrl+Q：退出；Ctrl+C：停止任务',
        )))
