...
with os.popen(adbPath+' shell wm size')as p:
    pixelOffset,androidScale=(lambda size:((0,round(960*size[1]/size[0])-540),1920/size[0])if size[0]*1080<size[1]*1920else((round(540*size[0]/size[1])-960,0),1080/size[1]))((lambda x:[int(i)for i in x])(re.search('[0-9]{1,}x[0-9]{1,}',p.read()).group().split('x')))
