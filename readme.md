...
> 福袋x毛酒吞双黄+c子  
> caber池子有点毒,最终氪了一单半总消耗46呼符+750石出了月神c呆双黄  
> 并附带长江秦良玉贝奥武夫红AC闪大姐章西女王卡米拉双子等共计11个四星  
> ylsfm,想要抽卡真就得老老实实准备1000石头  
...
***
# 使用说明 Instruction
这个项目的设计初衷是打破当前游戏版本下想尽办法3t速刷的固有思维,回归刚开服时克制队xjbd的环境,以达到不浪费羁绊点数和御主礼装经验的目的,在大量90+阴间本的环境下,xjbd能带给你最阳间的体验.尽管本项目也可用于3t,但**如果你是为了3t而来,就另请高明吧!**  
速览脚本功能,运行`fgoGui.py`,无需填写任何配置,去冬木大桥刷一根凶骨吧!  
大部分功能说明也在ui里,只有未在ui中记载的部分被记录在本文档后续的说明内容中  
ui大概长这样(设计视图仅供参考):  
![ui](doc/ui.png)  
这个脚本能识别技能和宝具是否可以使用,指令卡颜色和克制关系,无法行动及指令卡封印状态,可以半路接管战斗,会依据你的设置比较智能地帮你筛选助战,放技能,放宝具,吃苹果,依据特殊的选卡算法选卡,合理设定后可以无脑通过绝大部分非高难关卡,实战7-12回合能够刷完无限池终本  
...
[知乎付费咨询](https://www.zhihu.com/consult/people/1149733177244516352)亦可,但是只能在手机上操作,有点麻烦  
现在这个脚本居然已经可以从各大搜索引擎上找到,这就导致有很多halloworld都不会的小白来找我  
...
该功能还需进一步测试,保证鲁棒性后才能真正用于战斗策略,比如自动选取优先攻击对象  
...
另:先前提到的那个在B站发布网盘更新的3t脚本增加了一个根据编队界面进行从者识别的功能用于自动加载技能图标的特性,我在想要不要抄一个过来,这样的话或许真就不需要任何配置只要点开始按钮就行了...但是这会增加很多数据,并且需要跟着游戏进度更新,同时重写UI以及配置管理  
但是如果真的做了这个功能那么我是不是要向[Fgo Teamup](https://www.fgo-teamup.com/#/)一样再对每个活动图都建立配置呢?这样的话感觉就有点悖离本脚本设计初衷了...要做估计得另外开个FGO-teamSolve来求解编队  
...
8月3日B站上有个人写了个脚本发了个视频大约3.5w播放,但是他*不会用git*,所以仅仅以网盘的方式分发了源码和可执行文件,因而如果其视频的观众试图搜索FGO脚本,尤其是在GitHub上搜索,就有相当高的可能性看到本项目  
尽管这样能够获得很多播放/三连/关注与些许充电,但我不得不说这个老哥胆子是真的大,轻易就做到了我不敢做的事,敢在叔叔的地盘上这么搞,希望他别被查水表  
以此为鉴,在LICENSE中加入了不允许将本脚本相关内容发送到bilibili的条款  
## 2021/08/09
...
反正比什么都没有要好,之后会看看有没有更好的方法,比如直接通过tcp forward去ping手机上的server,同时重构本脚本的异常处理逻辑  
...
更新:移除了我自己编写的swipe  
更新:对于游戏画面不在屏幕中央的设备,在屏幕旋转后可以在「控制」菜单中刷新方向而无需重新建立连接  
## 2021/06/11 v6.1.3
更新:<无>  
由于本人经常处于手机开热点打fgo笔记本连热点挂脚本的情况,故在选取设备时输入*某四个特殊字母*能够获取并快速连接到网关ip,免得手动开终端ipconfig复制黏贴  
最近才想起来要加这么一个功能省下每次开机后的十秒时间,这个功能说实话打算藏在我自己的分支里不发布的,毕竟并不是所有人都在这个场景下使用,更不可能所有人的无线网络适配器都叫一个名字,所以这并不作为一个正式的脚本特性存在,仅仅作为一个类似彩蛋的东西,在代码里动了一点小手脚,如果恰好你也能用上那就凑合着用吧.这个功能与fgo没有关系,不会正儿八经的照着软件开发的样子做  
注意:调整了一些按键并未经充分测试  
## 2021/06/07 v6.1.2
...
更改:移除了「重新建立连接」选项  
现在直接更改到当前设备就会重新建立连接  
这「移动至后台」的仙鹤小姐真是拉满了yls的恶意...本脚本不打算为其机制提供额外支持  
还有一些细节优化  
## 2021/04/20 v6.1.0
更新:更改了连接设备的UI,现在你可以直接连接到远程设备,只需要在原先的选择设备对话框里输入对应地址  
同时,现在连接上的设备会显示出来  
更改:不WrapLongRows  
你uic有bug我就不用咯  
另:我发现airtest有严重的内存泄漏与僵尸线程问题,请尽量不要异常断开连接或是切换设备,如果你发现本脚本异常占用系统资源,目前只能关闭整个脚本进程重开  
我近期都会很忙,还入了Chia的坑,没时间给网易擦屁股  
## 2021/04/16 v6.0.3
优化:调整了检测卡色的算法  
之前为了方便就求个平均看看RGB哪个大,并取得了不错的效果  
但是有些从者指令卡--比如Kingprotea第二再临--是整块的绿色,在指令卡上下运动至接近最低点时就会显著影响分类结果  
尽管就算分类错误也不会导致脚本运行中断,我还是换用了靠谱的方法,毕竟这个脚本本体已经基本完善了  
## 2021/04/08 v6.0.2
更新:为日志输出上色  
