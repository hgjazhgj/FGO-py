...
下载release - 运行`FGO-py.bat` - 连接设备 - 游戏中把你要刷的关卡放在第一个 - 关闭`技能使用确认` - 点击「肝!」按钮
![关闭技能使用确认](doc/turnOffSkillPrompt.png)
...
![findWindowsApp](doc/getWindowsAppHandler001.png)
2. 按下回车后,FGO-py会枚举鼠标指针正下方的可见窗口及其所有子窗口.确认弹出窗口中展示的截图是fgo画面后,按照弹出窗口标题中的说明,按`i`判断目标窗口是否可以成功响应鼠标事件
![findWindowsApp](doc/getWindowsAppHandler002.png)
3. 如果能够成功响应,直接在弹出的窗口中按`y`,或在「连接到设备」对话框中输入弹出窗口标题中的句柄即可.例子中句柄值为`0xe16da`
![findWindowsApp](doc/getWindowsAppHandler003.png)
4. 如果不能成功响应,则检查目标窗口是否**具有管理员权限**.如果是,则FGO-py也需要以管理员身份运行,设置如下所示
![findWindowsApp](doc/getWindowsAppHandler004.png)
5. 窗口在战斗运行时不能被最小化或隐藏
6. 需要Windows 10版本1703或更高  
