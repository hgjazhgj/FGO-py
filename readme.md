...
以上库均可`pip install`,其他已包含在这些库里面的外部库未单独列出  
...
## 性能 Performance  
自古以来,获取识别用的图像是整个脚本中除去主动延时之外最消耗时间的操作  
这一开销大到其他任何算法优化都没有一丁点意义  
adb截图在模拟器上1.2fps,真机上0.5fps完全是没法用的状态  
曾经用过的win32截图30fps,是一种非常高效的截图方案,但是用起来很麻烦,得多一堆窗口句柄  
当前使用airtest库进行截图,模拟器上10.7fps,真机上3.7fps,还说得过去,也不怎么吃性能,用起来也舒服,其实这样的效率已经不影响使用了  
但是我希望找到一种截图方法,能够达到25fps以上,以至于几乎画面的每一帧都能被捕获  
同时每一帧图像又必须是独立的,不能作为视频编码,以保证画质,所以scrcpy之类的东西是不能用的  
若一定要以视频方式串流编码,码率至少为20Mbps  
STF的minicap宣称能达到30FPS,但是很遗憾兼容性很差,绝大多数模拟器和一些手机都用不了  
...
修bug  
增加一些独立的小工具  
建立能够详细描述战斗相关各种信息的数据模型  
我认为,fgo战斗的最大单位是从者,从者=数据+若干技能+一个宝具+指令卡,宝具和技能技能有若干效果,效果亦可嵌套效果,效果是施加于从者上的,效果即为战斗的最小单位  
所有单位都是callable的,但外部不应直接call效果对象  
...
+ 花花(这是一个相当好的例子)  
	...
	+ 宝具  
		+ 数据(卡色,hit,倍率,伤害分布等)  
		+ 效果:Buster性能提升  
		+ 效果:根据另一效果的数量提升Buster性能  
		+ 效果:敌方HP减少(造成群体伤害)  
		+ 效果:获得暴击星(宝具打星)  
		+ 效果:获得NP(宝具NP回收)  
	+ 指令卡  
		+ 数据(卡色,hit,伤害分布,暴击率,*兽的足迹*等)  
		+ 指令纹章(指令纹章必定*是*一个带若干效果的效果,没有数据内容)  
			+ 效果:为指令卡附加效果  
				+ 效果:敌方HP减少(附加伤害)  
		+ 效果:敌方HP减少(造成伤害)  
		+ 效果:获得暴击星(打星)  
...
各层级既可以向下访问,如从者HP减少会增加宝具伤害;可以平行访问,如一个技能减少了另一个技能的CD;亦可向上访问,如技能的即死效果消灭了从者  
仔细一想就知道这个模型相当不合理,比如指令纹章应在从者init时调用,而不应依附于指令卡上  
什么是面向对象程序设计啊...  
## 4.x  
[宝具伤害计算器](https://bbs.nga.cn/read.php?tid=20287657)现在又开始更新了,由于没有授权,我只能在这里放一个链接  
感谢大佬(就算又停更了,也还有一年可以用  
终于有现成的拿来就能用的fgo数据库了  
而且这个计算器的功能远远不止宝具信息,fgo里涉及战斗的数据全部都有  
当然我还是希望能把这些数据打包成json之类更易于访问的格式,这可能就是我该做的事了  
里边的计算功能也相当全,甚至我不需要关心里面到底有什么数据只要拿输出结果就可以了  
这也就意味着我根本不用管里面错综复杂的数据引用和挤在一格里面的一串if  
不得不佩服初代作者的毅力  
但是这个计算器是拿来计算输出的,防御/回复/控制等辅助技能的信息一概缺失  
所以搞不好我还得对这些数据大动干戈  
各大wiki上都只有一堆文本,GitHub上几乎所有相关项目都已经吃灰数载,难顶  
300个从者也不是很多嘛~  
等数据搞好之后我基本只要选定队伍中的从者id就能进行战斗了  
但这样还是有相当多的问题,比如技能具体向哪个目标施放  
所以对战斗形势的判断还有赖于更多更多更多的图像识别  
