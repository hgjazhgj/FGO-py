...
`fgoGui.py`也是脚本的一部分,会自动加载一些配置,在您知晓本脚本的工作原理前不建议单独使用`fgoFunc.py`进行操作  
理论上讲,你只需要想办法重写fgoFunc.Base类,就能用于iOS端  
在多数情况下,你很可能只需要更改图片模板就能用于其他语言的FGO版本  
...
又tm有憨憨上来就"大佬我不会用请教教我",*如果你瞬间理解了这句话的含义,[就不必看本部分剩下的内容](#助战-Friends)*  
...
显然直到您的问题解决为止我都很难定一个价格,那么只要您来我就收一颗圣晶石,之后的您看着给  
...
连代码都不会写难道得从helloworld教起吗,怎么想都不是我的问题吧  
